...
class fastglcm_wrapper:
    '''
    The script is named as texture.py. In this script this function 'fastglcm_wrapper' is 
    the same as the fast_glcm code found in github it just loops around for multiple 
    channels.
    Additionally I'm dividing each of the descriptive statistics by levels**2 to 
    normalise the values
    Example usage:
    tex2 = fastglcm_wrapper(im,levels=8,kernel_size=5,distance_offset=5,angles=[0,45,90,135])
    plot_rgb(tex2.calculate_glcm_entropy(),rb=2,gb=1,bb=0)
    '''
    ...
    def calculate_glcm(self):
        ...
        for i in range(k):
            ...
            # Inside your calculate_glcm function or wherever the large array is created
            #glcm_channels.append(np.mean(np.stack(glcm_angles, axis=-1), axis=-1).astype(np.float16))
