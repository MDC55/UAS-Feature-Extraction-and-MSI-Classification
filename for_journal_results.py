...
#
top_path='F:/2023 SA Fynbos Field Work/Drone data analysis code/'
...
#load raster1
def raster_clip(raster_path, raster_filename):
...
#%%
df1=raster_clip(raster_path1, raster_filename1)
...
correlation=texture_x(df1)

#%%
import matplotlib.pyplot as plt

def plot_single_correlation(correlation_channel, label):
    plt.imshow(correlation_channel, cmap='terrain', vmin=-1, vmax=1)
    plt.colorbar(label='Correlation')
    plt.title(f'GLCM Correlation - {label}')
    plt.show()

# Assuming correlation is a 2D NumPy array for a specific GLCM channel
# and you have a label for that channel
# Adjust the channel index and label based on your specific use case
channel_index = 3 # Adjust this index based on the channel you want to plot
channel_label = 'Channel 1'  # Adjust this label based on your specific use case

correlation_channel = correlation[:, :, channel_index]  # Assuming correlation is a 3D array
plot_single_correlation(correlation_channel, channel_label)
